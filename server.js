...
app.post('/baixar-dados', async (req, res) => {
  try {
    ...
    if (!clicou) {
      ...
      // Clicar no Botão "Exportar operações" apenas uma vez
      await page.evaluate(() => {
        const botoes = Array.from(document.querySelectorAll('button'));
        const botao = botoes.find(b => b.textContent.includes('Exportar operações'));
        if (botao) {
          botao.click();
          console.log('Exportação da segunda busca iniciada.');
        } else {
          throw new Error('Botão "Exportar operações" não encontrado.');
        }
      });
      ...
      try {
        ...
        const pastaDownloadsUsuario = path.join(os.homedir(), 'Downloads');
        const destinoFinal = path.join(pastaDownloadsUsuario, nomeFinal);
